...
func GetHTTPRequestResourceSchema() schema.Schema {
	return schema.Schema{
		...
		MarkdownDescription: "Represents an HTTP request resource, allowing configuration of various " +
			"HTTP request parameters and capturing the response details.\n\n" +
			"```hcl\n" + examples.GetHTTPRequestResourceExample() + "\n```\n\n" +
		...
		Attributes: map[string]schema.Attribute{
			...
			"id": helpers.ComputedStringAttribute(
				"A unique identifier for the resource. Format: `<RANDOM UNIQUE STRING>/<PARAMETERS ENCODED IN BASE64>`. " +
...
func (it *HTTPRequestResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if len(model.ID.ValueString()) == 0 {
		...
		// nosemgrep: go.lang.security.audit.crypto.sha224-hash.sha224-hash
		random := fmt.Sprintf("%x", sha256.Sum224([]byte(timestamp)))
		model.ID = types.StringValue(random)
	}
	if resp.Diagnostics.HasError() {
		return
	}
...
func unmarshalJSON(responseBody []byte, diagnostics *diag.Diagnostics) (map[string]interface{}, error) {
...
func decodeImportPayloadToModel(importPayload string, diagnostics *diag.Diagnostics) *HTTPRequestResourceModel {
	// Format: <RANDOM UNIQUE STRING>/<PARAMETERS ENCODED IN BASE64>
	...
	if len(parts) != AmountOfPartsInID {
		diagnostics.AddError(
			...
			"Expected a string with the format <RANDOM UNIQUE STRING>/<PARAMETERS ENCODED IN BASE64>.",
		)
		return nil
	}
