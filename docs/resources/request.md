...
- `ignore_changes` (Set of String) Optional list of attribute paths that should not force replacement when they change. Supports full attributes (e.g., `request_body`), individual map entries (e.g., `headers.X-Correlation-Id`), and JSON paths inside request bodies (e.g., `request_body.metadata.trace_id`).
