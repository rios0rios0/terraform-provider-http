---
name: code-review
description: "Review pull requests and diffs in terraform-provider-http — the Terraform provider that performs HTTP requests and stores responses in state — against the rios0rios0/guide standards, with extra weight on schema stability, state secrecy, and generated documentation. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `terraform-provider-http`

`terraform-provider-http` executes HTTP calls and stores the responses in Terraform state so later requests can use them, complementing `hashicorp/http`. Provider schemas are a public contract and state files are read by many people, so schema changes and response handling are the two highest-risk areas.
...
## What matters most in `terraform-provider-http`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **A schema change is a breaking change.** Renaming, removing, or re-typing an attribute in `internal/provider/` breaks every configuration in the field. It needs the three-place breaking-change flag, a MAJOR bump, and — where possible — a deprecation period rather than a removal.
- **Responses land in Terraform state, which is frequently unencrypted.** Any attribute that can carry a token, a cookie, or a credential must be marked sensitive. An unmarked response body that can contain secrets is a Critical finding.
- **`docs/` is generated, not hand-written.** A schema change without regenerated documentation, or a manual edit to a generated page, is a finding. `examples/` must keep compiling against the new schema.
- **Request behaviour must be deterministic and bounded** — explicit timeouts, a bounded retry policy, and no unbounded body reads. A provider that hangs blocks the whole plan.
- **Errors surface as Terraform diagnostics**, with the URL and status code but without the credentials used to make the call.
- **`.goreleaser.yml` and `terraform-registry-manifest.json` are release-critical.** A change to either affects Registry publication and needs the release path checked, not just the build.
- Keep the `internal/domain` / `internal/infrastructure` / `internal/provider` split: provider plumbing adapts, the domain decides.

### Commands a reviewer should be able to quote

```bash
make all                  # lint + semgrep + gitleaks + test
make lint && make test    # this Makefile has no `sast` target; use the per-tool ones
make semgrep && make gitleaks
go test ./internal/...
terraform -chdir=examples/resources plan   # after a schema change
```
...
- HTTP behaviour is proven against a real local test server (`httptest.NewServer`), never against a transport mock.
- Acceptance tests under `test/infrastructure/` cover the resource lifecycle: create, read, update, delete, and import.
...
- The generated files under `docs/` differing in whitespace from a hand edit — regenerate rather than debating them.
